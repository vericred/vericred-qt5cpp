...
void
SWGProvidersApi::getProviders_1(SWGRequestProvidersSearch body) {
    QString fullPath;
    fullPath.append(this->host).append(this->basePath).append("/providers/search/geocode");
    ...
    connect(worker,
            &HttpRequestWorker::on_execution_finished,
            this,
            &SWGProvidersApi::getProviders_1Callback);

    worker->execute(&input);
}

void
SWGProvidersApi::getProviders_1Callback(HttpRequestWorker * worker) {
    QString msg;
    if (worker->error_type == QNetworkReply::NoError) {
        msg = QString("Success! %1 bytes").arg(worker->response.length());
    }
        ...
        QString json(worker->response);
    SWGProvidersGeocodeResponse* output = static_cast<SWGProvidersGeocodeResponse*>(create(json, QString("SWGProvidersGeocodeResponse")));
    

    worker->deleteLater();

    emit getProviders_1Signal(output);
