...
void
SWGNetworkSizesApi::searchNetworkSizes(SWGStateNetworkSizeRequest body) {
    QString fullPath;
    fullPath.append(this->host).append(this->basePath).append("/network_sizes/search");
    ...
    connect(worker,
            &HttpRequestWorker::on_execution_finished,
            this,
            &SWGNetworkSizesApi::searchNetworkSizesCallback);

    worker->execute(&input);
}

void
SWGNetworkSizesApi::searchNetworkSizesCallback(HttpRequestWorker * worker) {
    QString msg;
    if (worker->error_type == QNetworkReply::NoError) {
        msg = QString("Success! %1 bytes").arg(worker->response.length());
    }
    ...
    emit searchNetworkSizesSignal(output);
