...
void
SWGNetworksApi::showNetwork(qint32 id) {
    QString fullPath;
    fullPath.append(this->host).append(this->basePath).append("/networks/{id}");
    ...
    connect(worker,
            &HttpRequestWorker::on_execution_finished,
            this,
            &SWGNetworksApi::showNetworkCallback);

    worker->execute(&input);
}

void
SWGNetworksApi::showNetworkCallback(HttpRequestWorker * worker) {
    QString msg;
    if (worker->error_type == QNetworkReply::NoError) {
        msg = QString("Success! %1 bytes").arg(worker->response.length());
    }
        ...
        QString json(worker->response);
    SWGNetworkDetailsResponse* output = static_cast<SWGNetworkDetailsResponse*>(create(json, QString("SWGNetworkDetailsResponse")));
    

    worker->deleteLater();

    emit showNetworkSignal(output);
