...
SWGZipCountiesApi::getZipCountiesCallback(HttpRequestWorker * worker) {
    ...
    SWGZipCountiesResponse* output = static_cast<SWGZipCountiesResponse*>(create(json, QString("SWGZipCountiesResponse")));
...
void
SWGZipCountiesApi::showZipCounty(qint32 id) {
    QString fullPath;
    fullPath.append(this->host).append(this->basePath).append("/zip_counties/{id}");
    ...
    connect(worker,
            &HttpRequestWorker::on_execution_finished,
            this,
            &SWGZipCountiesApi::showZipCountyCallback);

    worker->execute(&input);
}

void
SWGZipCountiesApi::showZipCountyCallback(HttpRequestWorker * worker) {
    QString msg;
    if (worker->error_type == QNetworkReply::NoError) {
        msg = QString("Success! %1 bytes").arg(worker->response.length());
    }
    ...
    emit showZipCountySignal(output);
